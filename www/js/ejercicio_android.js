...
var AppTexts = function() {
    ...
    ModalWindow = function(a) {
        ...
        if (e || c) e = Math.ceil(f * _boardObj.getSize().width), f = $('<DIV id="modal_window_close"><IMG src="' + _siteUri + 'images/buttons/mw-close.png" id="modal_window_close_img">'), k.append(f), $("#modal_window_close").on("click", function() {
                d();
                return !1
            }), $("#modal_window_close_img").width(e),
            $("#modal_window_close_img").height(e);
        c && (e = Math.ceil(.04 * _boardObj.getSize().width), f = $('<DIV id="modal_window_yes_buttons" class="modal-window-yes-panel"><DIV id="modal_window_yes"><IMG src="' + _siteUri + 'images/buttons/mw-button-yes.png"></DIV>'), k.append(f), $("#modal_window_yes").on("click", function() {
            c();
            d();
            return !1
            ...
            a.get = function(d, g) {
                ...
                var m = $("#param_main_link_token").val();
                    ...
                    h ? ajax({
                            ...
                            url: URL_SERVER+"getEjercicioObject/"+h,
                            async: !1,
                            beforeSend: function() { $("#bg_loading_link_data_store").show() },
                            success: function(e) {                       
                                a.data.animation = e;
                                a.data.animation.edit_mode = document.querySelector("#param_animation_edit_mode") ? +document.querySelector("#param_animation_edit_mode").value : 0 ;
                                $("#btn_animation").css("display", "none");
                            },
                            complete: function() {
                        ...
                        dataType: "json",
                        headers: {"Authorization": localStorage.getItem("LS_USER_API_KEY")},
                        url: URL_SERVER+"getEjercicioObject/"+m,
                        async: !1,
                        beforeSend: function() { $("#bg_loading_link_data_store").show() },
                        success: function(e) { a.data.main = e; },
                        complete: function() { $("#bg_loading_link_data_store").hide() },
                        error: function(e) {
                            var d = _projectTexts.get();
                            console.log(e);
                            a.data.main = {};
...
var BoardHelpers = {
        ...
        boardsConfigs: {
            "PADEL": {bgColor: "#738554", strokeColor: "#ffffff", heightToWidthCoeff: 0.6568, playerToBoardWidthCoeff: 0.026184}
        }
    },
    Board = function(J) {
        var L = J.type,
        ...
        function q() {  // ESTA FUNCION CONTROLA LAS DIMENCIONES DE LA PISTA 
            var a = window.innerWidth, // +(.97 * window.innerWidth).toFixed(),
                b = window.innerHeight, //+(.97 * window.innerHeight).toFixed(),
                e = a,
                c = 200;
            ...
            h.width  = a; //Math.ceil(ANCHOMONITOR);
            h.height = b; //Math.ceil(ALTOMONITOR);
        }

        function w() { // ESTA FUNCION AÑADE LOS TAMAÑOS AL SVG
            var ALTOMONITOR = document.getElementById('main_body').offsetHeight;
                ALTOMONITOR = (ALTOMONITOR + 55).toFixed();
            
            u || (u = document.getElementById("svg_canvas"));
            u.setAttribute("width", h.width);
            u.setAttribute("height", h.height);
            u.setAttribute("viewBox", "0 0 " + h.width + " " + ALTOMONITOR);
            ...
            (function() {
                ...
                if (a && !b) {
                    for (var c = 0; c < AppConfig.shapesColors.length; c++) {
                        ...
                        b.setAttribute("src", _siteUri + "images/buttons/shapes/" + l + ".png");
        ...
        function E() {
            ...
            (function() {
                ...
                document.getElementById("btn_animation").addEventListener("click", function() { 
                    ...
                    $("#btn_Anterior_Menu_Fijo").hide();
                    $("#btn_Siguiente_Menu_Fijo").hide();
                }, !1);
                
                document.getElementById("btn_back").addEventListener("click", function() { 
                    BoardHelpers.showPage("main"); 
                    localStorage.setItem('LS_TYPE', "MAIN"); 
                    $("#btn_Anterior_Menu_Animation").hide();
                    $("#btn_Siguiente_Menu_Animation").hide();

                    $("#btn_Anterior_Menu_Fijo").show();
                    $("#btn_Siguiente_Menu_Fijo").show();
                }, !1);
            })();              
            (function() {
    ...
    Player = function(J) {
        ...
        function x(a, b) {
            function f(a) {
                if (!n) {
                    ...
                    b < +(2 * e).toFixed() ? b = +(2 * e).toFixed() : parseInt(k.getAttribute("y")) <= _boardObj.getSize().height - 1.6 * e ? b > _boardObj.getSize().height - 1.6 * e && (b = _boardObj.getSize().height - 1.6 * e) : a > .45 * _boardObj.getSize().width && (a = .45 * _boardObj.getSize().width);
                    var d = a+20,
                        f = b;
                    w(d, f, k);
                    C(d, f, r);
        ...
        function G(a, b) {
            function f(a) {
                ...
                a < +(2 * n).toFixed() ? a = +(2 * n).toFixed() : parseInt(k.getAttribute("y")) <= _boardObj.getSize().height - 1.6 * n ? a > _boardObj.getSize().height - 1.6 * n && (a = _boardObj.getSize().height - 1.6 * n) : b > .45 * _boardObj.getSize().width && (b = .45 * _boardObj.getSize().width);
                var d = b,
                    f = a;
                w(d, f, k);
                C(d, f, r);
        ...
        h.refresh = function(b) {
            if (a) {
                var c = Math.ceil(_boardObj.getSize().width * u); // Radio circulos
                    b = +(_boardObj.getSize().width * B.left).toFixed(); // + 20;
                    f = +(_boardObj.getSize().height * B.top).toFixed();
                    d = +(.67 * c).toFixed();
    ...
    Ball = function(J) {
        ...
        c.createClone = function(a, b) {
            ...
            c.attr("opacity", "0.5");
            c = document.getElementById(a + e + "_shape");
            ...
            for (var c = _boardObj.getSize().width * _boardObj.getPlayerToBoardWidthCoeff(), h = +(_boardObj.getSize().height - .6 * c).toFixed(), n = 0; n < AppConfig.playersColors.length; n++)
                for (var e = AppConfig.playersCount; 0 < e; e--) {
                    ...
                    var l = Math.ceil(n * c) + Math.ceil(c / 0.35) * n;
                        ...
                        l = Math.ceil(l);                    
                        l = {
                            top: + (h / _boardObj.getSize().height).toFixed(30),
                            left: + (l / _boardObj.getSize().width).toFixed(30)
                        };
    ...
    ShapesPainter = function(J) {
        ...
        c.enable = function() {
            if (m && "null" != m) {
                B = !0;
                var a = _siteUri + "images/cursors/pencil.png";
        ...
        c.setShapeType = function(a) {
            (m = a) && "null" != a ? (B = !0, a = _siteUri + "images/cursors/pencil.png", document.querySelector("#" + h).style.cursor =
    ...
    Main = function(J) {
        var G = (J || {}).link_token || null,
            ...
            c = null,
            h = null,
            n = { players: Object.create(null), balls: Object.create(null) },
            e = null;
        ...
        function x() {
            e || (e = {
                ...
                clearAll: function(b) {
                    b.preventDefault();
                    "localStorage" in window && null !== window.localStorage && localStorage.removeItem("main_" + _boardObj.getType());
                    v.reset();
                    c.reset();
                    h.clean();
                    n = {
                    ...
                    ajax({
                        ...
                        success: function(a) {
                            if (c) a = _projectTexts.get(), new ModalWindow({
                                ...
                                b.style.fontSize = Math.ceil(.013092 * _boardObj.getSize().width) + "px";
        ...
        (function() {
            ...
            document.querySelector("#context_menu_player_main").addEventListener("keypress", x().contextMenuKeyPress, !1);
            ...
            document.querySelector("#clear_all_button").addEventListener("click", x().clearAll, !1);
            document.querySelector("#btnDeleteEjercicio").addEventListener("click", x().clearAll, !1);
            document.querySelector("#addEjercicioFijo").addEventListener("click", x().clearAll, !1);
            h = new ShapesPainter({
                contextMenu: {
                ...
                onShapeAdded: function() { w() },
                onShapeChanged: function() { w() },
                onShapeRemoved: function() { w() }
            });
            v = new Players({
                contextMenu: {
        ...
        this.show = function(b) {
            ...
            c.refresh();
            L();
            document.querySelector("#shapes_buttons_panel #remove_last_shape").addEventListener("click", x().shapesPainterRemoveLastShape, !1);
        ...
        this.getPlayers = function() { return v };
        this.getBalls = function() { return c };
        this.getShapesPainter = function() { return h }
    },
    Animation = function(J) {
        ...
        function L() {  // ESTA FUNCION AGREGA TODOS LOS EVENTOS
            ...
            document.querySelector("#btn_animation_speed_plus").addEventListener("click", function() { m(S + 1) }, !1);
            document.querySelector("#btn_animation_speed_minus").addEventListener("click", function() { m(S - 1) }, !1);
            ...
            document.querySelector("#list_ejercicios").addEventListener("click", G().resetAnimationButtonClick, !1);

            $("body").on("click", "li.liEjercicio", function(){
                var EDIT = $(this).data("edit");
                initGetAnimation(EDIT);
            });
        ...
        function G() { // FUNCIONES EXTRAS PARA LOS BOTONES
            qa || (qa = {
                ...
                resetAnimationButtonClick: function(a) { A(); },
                        ...
                        success: function(a) {
                            if (b) a = _projectTexts.get(), new ModalWindow({
                                ...
                                document.queryCommandSupported("copy") ? (document.querySelector("#copy_animation_link_button").addEventListener("click",
                                    ...
                                    path: _siteUri + "js/ZeroClipboard.swf",
                ...
                frameElementMove: function(a, b) { // ESTA FUNCION CREA LA LINEA CUANDO SE MUEVE EL OBJETO
                    ...
                    if (void 0 !== g[H].elements[a].pos.s) {
                        ...
                        var e = $("#" + a).width() + 95 / 2 + _boardObj.getSize().width * c.x,
                            f = $("#" + a).width() + 55 / 2 + _boardObj.getSize().height * c.y,
                            c = e + "," + f,
                            e = $("#" + a).width() + 95 / 2 + _boardObj.getSize().width * b.x,
                            f = $("#" + a).width() + 55 / 2 + _boardObj.getSize().height * b.y;
            ...
            for (b in g[a].elements)
                if (g[a].elements.hasOwnProperty(b)) {
                    ...
                    var c = {
                        ...
                        left: g[a].elements[b].pos.e.x + 0.028
                    };
                    ...
                    void 0 !== g[a].elements[b].pos.im1 && !0 !== aa && !0 !== ga && (1 == R && (0 === b.indexOf("animation_player_") && O.createPlayerClone(b, "anim_clone_", g[a].elements[b].pos.s), 0 === b.indexOf("animation_ball_") && Q.createBallClone(b, "anim_clone_", g[a].elements[b].pos.s), d(b, a), U(b, a)), f(b, a))
                }
            N.removeAdditionalShapes();
            ca();
                ...
                for (var b = 0; b < g.length; b++)
                    for (var d in g[b].elements)
                        if (g[b].elements.hasOwnProperty(d) && (g[b].elements[d].element = document.querySelector("#" + d), g[b].elements[d].lineWayPoints = void 0, void 0 !== g[b].elements[d].pos.im1)) {
                            var e = [];
                            e.push({
                                x: g[b].elements[d].pos.s.x + 0.034,
                                y: g[b].elements[d].pos.s.y
                            });
                            e.push({
                                x: g[b].elements[d].pos.im1.x + 0.034,
                                y: g[b].elements[d].pos.im1.y
                            });
                            e.push({
                                x: g[b].elements[d].pos.im2.x + 0.034,
                                y: g[b].elements[d].pos.im2.y
                            });
                            e.push({
                                x: g[b].elements[d].pos.e.x + 0.034,
        ...
        function f(a, b) { // ESTA FUNCION CREA LA LINEA DEL MOVIMIENTO  + 0.045
            ...
            for (var c = ba(c, 13), d = [], e = $("#" + a).width(), f = 0; f < c.length; f++) d.push(e + 95 / 2 + _boardObj.getSize().width * c[f].x + "," + (e + 55 / 2 + _boardObj.getSize().height * c[f].y));
        ...
        function U(a, b) { // ESTA FUNCION CREA LOS PUNTOS DE LA LINEA DEL MOVIMIENTO
            var c = "#555",
                ...
                g[b].elements[a].circle_im1.attr("cx", e  + 95 / 2 + _boardObj.getSize().width * g[b].elements[a].pos.im1.x);
                g[b].elements[a].circle_im1.attr("cy", e  + 55 / 2 + _boardObj.getSize().height * g[b].elements[a].pos.im1.y);
                ...
                g[b].elements[a].circle_im2.attr("cx", e  + 95 / 2 + _boardObj.getSize().width * g[b].elements[a].pos.im2.x);
                g[b].elements[a].circle_im2.attr("cy", e  + 55 / 2 + _boardObj.getSize().height * g[b].elements[a].pos.im2.y);
        ...
        (function() {
            L(); // ESTA FUNCION AÑADE LOS EVENTOS
            N = new ShapesPainter({
                contextMenu: {
        ...
        function initGetAnimation(IS_EDIT){
            _isEditable = IS_EDIT;
            cargarEjercicio();    
            isEditable(IS_EDIT);
        };
...
function savExercise() {
    ...
    if(localStorage.LS_ID_EJERCICIO === undefined){
        var MODAL = new ModalWindow({
            html: '<h4 style="color:#fff;">Tienes que crear o seleccionar un ejercicio antes de cualquier acción.</h4>'
        });
    }else{
        var MODAL = new ModalWindow({
            html: '<form class="form" id="formEjercicios"><input type="hidden" name="txtIdEjercicio" id="txtIdEjercicio" value="'+IDEJERCICIO+'" /> <input name="txtCodigoEjercicio" type="text" class="input feedback-input" placeholder="Codigo Ejercicio" id="txtCodigoEjercicio" value="'+CODIGO_EJERCICIO+'" /> <input name="txtTituloEjercicio" type="text" class="input feedback-input" placeholder="Nombre ejercicio" id="txtTituloEjercicio" value="'+TITULO+'" /> <textarea name="txtDescripcionEjercicio" class="textarea feedback-input" id="txtDescripcionEjercicio" placeholder="Descripcion del ejercicio">'+DESCRIPCION+'</textarea><input type="submit" value="Guardar" id="button-blue"/></form>'
        });
    }    

    /* *****  A C C I O N E S  ****** */
    $('#formEjercicios').on('submit', function(event){
        event.preventDefault();
        
        var TIPO     = localStorage.getItem('LS_TYPE');
        var ESENARIO = TIPO.toLowerCase();
        var OBJETOS  = localStorage.getItem(ESENARIO+"_PADEL");
        ...
        if($('#txtTituloEjercicio').val() == ''){
            new ModalWindow({ html: '<h1 style="color:#fff;"> Tienes que añadir un titulo al ejercicio. </h1>' });
        }else{
            var form_data = $(this).serialize() + "&txtObjetos="+OBJETOS+"&txtTipo="+TIPO;
            ...
            $.ajax({
                ...
                success: function (data){                    
                    //$('#formEjercicios')[0].reset();                    
                    
                    if(data.message == 'AP_INSERT'){
                        new ModalWindow({ html: '<h4 style="color:#fff;"> Ejercicio creado con exito. </h4>' });
                    ...
                    if(data.message == 'AP_UPDATE'){
                        new ModalWindow({ html: '<h4 style="color:#fff;"> Ejercicio actualizado con exito. </h4>' });
                    }
...
function loadEjercicios(){
    ...
    $("#divContentModal").html("");
    $("#divContentModal").append('<div class="list links-list"><ul id="ulEjercicios">');
    ...
    _ap_request = $.ajax({
		...
		success: function (data){
			$.each(data.Ejercicios, function(index, item) {
                HTML_1 += "<li class=\"liEjercicio\" data-edit="+item.edit+"><a href=\"#\" onclick=\"getEjercicio("+item.id+", '"+item.type+"');\">"+item.title+"</a></li>";			
			});   
    ...
    $("#divContentModal").append('</ul></div>');

    return false;
}

function OpenModalEjercicios(){
    var HTML_EJERSISIOS = $("#divContentModal").html();
    FINAL_MODAL = new ModalWindow({
        html: HTML_EJERSISIOS
    });

    return false;
}

function getEjercicio(ID, TIPO){
    ...
    var TIPO1  = TIPO.toLowerCase(); 
        ...
        _linkDataStore = new LinkDataStore;
        _linkDataStore.get(q, J);
        BoardHelpers.showPage(q, {link_token: J});
...
function existeObjeto(OBJETO, TIPO){

    var DATA = JSON.parse(localStorage.getItem(TIPO+"_PADEL"));
    var VALOR = false;
    ...
    if(TIPO == "main"){
        ...
        $.each(DATA.frames_elements, function(i, v) {
            //VALOR = $('#'+OBJETO ).attr('class').split(' ')[2];
            if(i == OBJETO){ VALOR = true; }
        });
    }
...
function anterior_Menu_Fijo(){    
    $('#players_section > .player_fijo').each(function() {
        var THIS      = this.id,
            ID        = $('#'+ this.id +'_shape'),
            POS_LEFT  = ID.attr("x");
            NUW_VALOR = parseInt(POS_LEFT) + 60; 

        var OBJETO = existeObjeto(THIS, "main");
...
function anterior_Menu_Animation(){  
    $('#players_section > .player_animation').each(function() {
        ...
        var OBJETO = existeObjeto(THIS, "animation");
...
function addEjercicio(TIPO){
    if(TIPO == "MAIN"){
        localStorage.setItem('main_PADEL', '{"players":{},"balls":{},"shapes":[]}');                
        localStorage.setItem('LS_TYPE', "MAIN");
        BoardHelpers.showPage("main", {link_token: null}); 
        ...
        localStorage.setItem('LS_TYPE', "ANIMATION");
        BoardHelpers.showPage("animation", {link_token: null}); 
...
function resetEjercicio(){
    ...
    localStorage.removeItem('LS_ID_EJERCICIO');
    localStorage.removeItem('LS_CODIGO_EJERCICIO');
    localStorage.removeItem('LS_TITLE_EJERCICIO');
    localStorage.removeItem('LS_DESCRIPTION_EJERCICIO');  
...
function comprobar_estado_tipo(TIPO){
    if( TIPO == "main"){
        $("#btn_Anterior_Menu_Animation").hide();
        $("#btn_Siguiente_Menu_Animation").hide();
...
function drawFieldMarkup(a, b, c) {
    ...
    a = ''; // '<path d="M10 10 H' + (a-10).toFixed() + ' V' + (VERTICAL_LINE).toFixed() + ' H10 V10 M' + (MITAD).toFixed() + ' ' + (POS_1).toFixed() + ' V' + (POS_2).toFixed() + ' M10 ' + (POS_1).toFixed() + ' H' + (a - 10).toFixed() + ' M10 ' + (POS_2).toFixed() + ' H' + (a - 10).toFixed() + ' Z" fill="none" stroke="' + c + '" stroke-linecap="square" stroke-width="' + d + '"/>';
